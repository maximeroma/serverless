...
function captureKinesis(event, context, sampleDebugLogRate) {
  ...
  const events = event.Records.map(parsePayload).map(record => {
    ...
    if (!recordContext["Debug-Log-Enabled"]) {
      recordContext["Debug-Log-Enabled"] =
        Math.random() < sampleDebugLogRate ? "true" : "false"
    }
